...
```sh
npm install @fnsdomains/fnsjs ethers
```
...
```js
import { FNS } from '@fnsdomains/fnsjs'
...
const batched = await FNSInstance.batch(
  FNSInstance.getText.batch('test.fil', 'foo'),
  FNSInstance.getAddr.batch('test.fil'),
  FNSInstance.getOwner.batch('test.fil'),
)
...
If you want to use your own graph-node URI, such as a local graph-node URI, you can pass it through when creating a new FNS instance.
...
const FNSInstance = new FNS({ graphURI: null })
```
...
**NOTE:**  
The profile function will always request an FIL addr record.
...
const profile = await FNSInstance.getProfile('test.fil')
...
const history = await FNSInstance.getHistory('test.fil')
...
const historyWithDetail = await FNSInstance.getHistoryWithDetail('test.fil')
```
...
```js
/* wrap a .fil name */
const tx = await FNSInstance.wrapName(
  'test.fil', // Name to wrap
...
The GQL manager is used as to separate the reliance of FNSjs from GQL.
It only loads in GQL when it is needed, or not at all if specified in the constructor of the FNS class.
Very simply, it just exposes the core functions needed for FNSjs which can then be accessed.
...
`import { encodeContenthash } from '@fnsdomains/fnsjs/utils/contentHash'`
...
- `coinType`: string? | number?
  - Target coin
  - Defaults to 60 (FIL) if undefined
